...
def extractFeaturesSep(tempFolder):
    ...
    for n, noteFile in enumerate(noteFiles):
        ...
        notes.append({
            ...
            "harmonicShort": [np.quantile(harmonic, i/100) for i in range(1, 101)],
            "harmonicAvg": np.mean(harmonic),
            "harmonicMax": np.max(harmonic),
            "harmonicMin": np.min(harmonic),
            "harmonicQ1": np.quantile(harmonic, 0.25),
            "harmonicQ2": np.quantile(harmonic, 0.5),
            "harmonicQ3": np.quantile(harmonic, 0.75),
            "harmonicVar": np.var(harmonic)
        })
